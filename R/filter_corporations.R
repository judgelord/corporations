...
#'
#' @param corporations_return_cols Optional vector of column names to include from the
...
#' @param search_term Optional character string; if provided, filters the
#' company names using a partial string match (case-insensitive).
#' @param verbose Logical; if TRUE, displays progress messages.
#'
#' @return A tibble containing the filtered subset of the corporations database,
#' including columns such as `aliases`, `ticker`, `cik`, and `naics`.
#' @export
