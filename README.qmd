...
`corporations` extracts regular-expression-based pattern matches from a vector of text using a built-in extensive crosswalk table of corporations. This includes companies with a central index key (a unique 10-digit, permanent identification number assigned by the U.S. Securities and Exchange Commission), Compustat (database managed by the S&P Global Market Intelligence), and FDIC-insured companies. The corprations_extract function requires one inputs
